...
import="com.model.Board" %> <%@ page import="com.model.BoardAttachment" %> <%
Board board = (Board) request.getAttribute("board"); String username = (String)
...
<html>
  <head>
    ...
    <title>글 수정 - Security Crawling</title>
    <link
      rel="stylesheet"
  ...
  <body>
    <div class="container">
      <div class="nav-links">
        ...
        <a
          href="<%= request.getContextPath() %>/board/view?id=<%= board.getBoardId() %>"
        >
          게시글</a
        >
      </div>
      ...
      <form
        action="<%= request.getContextPath() %>/board/edit"
        method="post"
...
<%= board.getContent() %></textarea
        ...
        <div class="form-group">
          ...
          <div class="existing-files">
            <% for (BoardAttachment attachment : board.getAttachments()) { %>
            <div class="file-item">
              <input
                type="checkbox"
                name="deleteFiles"
                value="<%= attachment.getAttachmentId() %>"
                id="file_<%= attachment.getAttachmentId() %>"
              />
              <label for="file_<%= attachment.getAttachmentId() %>">
                <%= attachment.getOriginalFilename() %> (<%=
                attachment.getFormattedFileSize() %>)
                <span style="color: #dc3545; margin-left: 8px">삭제</span>
              </label>
            </div>
            <% } %>
          </div>
