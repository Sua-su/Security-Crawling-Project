...
public class BoardAttachmentDAO {
    ...
    public boolean insertAttachment(BoardAttachment attachment) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            if (result > 0) {
                ...
                System.out.println("✅ 첨부파일 저장 성공 - ID: " + attachment.getAttachmentId() +
                        ", 파일명: " + attachment.getOriginalFilename());
                return true;
            }
            return false;

        } catch (SQLException e) {
            System.err.println("❌ 첨부파일 저장 실패");
            e.printStackTrace();
            return false;
        }
    }
    ...
    public List<BoardAttachment> getAttachmentsByBoardId(int boardId) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            System.err.println("❌ 첨부파일 목록 조회 실패 - board_id: " + boardId);
            e.printStackTrace();
        }
    ...
    public BoardAttachment getAttachmentById(int attachmentId) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            System.err.println("❌ 첨부파일 조회 실패 - attachment_id: " + attachmentId);
            e.printStackTrace();
        }
    ...
    public boolean incrementDownloadCount(int attachmentId) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            System.err.println("❌ 다운로드 횟수 증가 실패 - attachment_id: " + attachmentId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean deleteAttachment(int attachmentId) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            if (result > 0) {
                System.out.println("✅ 첨부파일 삭제 성공 - attachment_id: " + attachmentId);
                return true;
            }
            return false;

        } catch (SQLException e) {
            System.err.println("❌ 첨부파일 삭제 실패 - attachment_id: " + attachmentId);
            e.printStackTrace();
            return false;
        }
    }
    ...
    public boolean deleteAttachmentsByBoardId(int boardId) {
        ...
        try (Connection conn = DBConnect.getConnection();
            ...
            pstmt.setInt(1, boardId);
            int result = pstmt.executeUpdate();
            System.out.println("✅ 게시글의 첨부파일 " + result + "개 삭제 - board_id: " + boardId);
            return true;

        } catch (SQLException e) {
            System.err.println("❌ 게시글 첨부파일 삭제 실패 - board_id: " + boardId);
            e.printStackTrace();
            return false;
        }
    }
