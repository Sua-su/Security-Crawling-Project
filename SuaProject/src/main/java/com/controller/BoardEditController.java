...
public class BoardEditController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            // 권한 확인 (본인 또는 관리자만)
            if (!isAdmin && board.getUserId() != userId) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (title == null || title.trim().isEmpty() ||
                ...
                response.sendRedirect(request.getContextPath() + "/board/edit?id=" + boardId + "&error=" +
            ...
            boolean success = boardDAO.updatePost(board);
            ...
            if (success) {
                ...
                if (deleteFiles != null) {
                    for (String attachmentId : deleteFiles) {
                        BoardAttachment attachment = attachmentDAO.getAttachmentById(Integer.parseInt(attachmentId));
                        if (attachment != null) {
                            // 실제 파일 삭제
                            ...
                            // DB에서 삭제
                            attachmentDAO.deleteAttachment(Integer.parseInt(attachmentId));
                        }
                    }
                }

                // 새 첨부파일 처리
                handleFileUploads(request, boardId);
            ...
            response.sendRedirect(request.getContextPath() + "/board/list?error=" +
                    URLEncoder.encode("게시글 수정 중 오류가 발생했습니다.", "UTF-8"));
        }
    }
